...
Todesfaelle <- as.matrix(read_excel("COVID-19_Todesfaelle.xlsx",sheet = "COVID_Todesfälle_KW_AG10" , ))
...
Mege and order both data frames in order to receive homogenous data set
...
Altersverteilung <- Altersverteilung[,-(dim(Altersverteilung)[2]:(dim(Altersverteilung)[2]-1))]

X <- matrix(0, nrow = 10, ncol = 119)
rownames(X) <- rev(c("0-9","10-19","20-29","30-39","40-49","50-59","60-69","70-79","80-89","90+"))
colnames(X) <- colnames(Altersverteilung)
...
X is now the new Age distribution for cases over the period of the pandemic.
...
Calculating the Ratios per Age-Group over all time at first. Afterwards regardig the different time-periods 
...
Now modeling the deaths as a Binomial Model regarding Infections:
  ...
  \item Setting a Prior \rightarrow Beta Prior as Conjugate for Binomial
...
bayes_estimate <- (Y_sum + alpha_0)/(X_sum + alpha_0 + beta_0)

comparison_frame <- data.frame(bayesestimate = bayes_estimate,ratio = d_ratios, cases = X_sum) 
...
ggplot(data = comparison_frame, aes(x = log(ratio), y = log(bayes_estimate), colour = log(cases)))+
  geom_point()+
...
bayes_estimate_1W <- (Y_sum1W + alpha_0)/(X_sum1W + alpha_0 + beta_0)
comparison_frame_1W <- data.frame(bayesestimate1W = bayes_estimate_1W,ratio1W = d_ratios_1W, cases = X_sum1W) 
...
ggplot(data = comparison_frame_1W, aes(x = log(ratio1W), y = log(bayesestimate1W),colour = log(cases)))+
  geom_point()+
  geom_abline(lty = 2, col= "red", lwd= .2)+
  geom_text(
...
bayes_estimate_1D <- (Y_sum1D + alpha_0)/(X_sum1D + alpha_0 + beta_0)
comparison_frame_1D <- data.frame(bayesestimate1D = bayes_estimate_1D,ratio1D = d_ratios_1D, cases = X_sum1D) 

```

```{r}
ggplot(data = comparison_frame_1D, aes(x = log(ratio1D), y = log(bayesestimate1D),colour = log(cases)))+
  geom_point()+
  geom_abline(lty = 2, col= "red", lwd= .2)+
  geom_text(
...
bayes_estimate_2W <- (Y_sum2W + alpha_0)/(X_sum2W + alpha_0 + beta_0)
comparison_frame_2W <- data.frame(bayesestimate2W = bayes_estimate_2W,ratio2W = d_ratios_2W, cases = X_sum2W) 
...
```{r}
ggplot(data = comparison_frame_2W, aes(x = log(ratio2W), y = log(bayesestimate2W),colour = log(cases)))+
  geom_point()+
  geom_abline(lty = 2, col= "red", lwd= .2)+
  geom_text(
...
Using MCMC and Metropolis Hastings Algorithm to implement uniform(beta) uninformative prior

Using the data defined in the first wave and evaluating posterior for age group 0-9 :
...
Implementation with non-imformative uniform prior

````{r}
n.sims <- 1000 #number simulations
theta_0 <- runif(1, min = 0, max = .5)
theta_vec <- rep(0, n.sims+1) #create posterior vector
theta_vec[1] <- theta_0 #initialize posterior vector

for (t in 1:n.sims){

  theta_star <- runif(1, min = 0, max = .5) #proposal from prior    
  theta_t <- theta_vec[t] #previous accepted value for posterior  
  
  #Create MH-Ratio using log-likelihoods
  
  MH_Ratio <- sum(dbinom(group1_dframe, size = 1, log = T, prob = theta_star))/
    sum(dbinom(group1_dframe, size = 1, log = T, prob = theta_t))
...
Implementation with beta prior

```{r}


n.sims <- 10000 #number simulations
...
for (t in 1:n.sims){
  ...
  MH_Ratio <- dbinom(group1_dframe, size = 1, log = T, prob = theta_star)/
    dbinom(group1_dframe, size = 1, log = T, prob = theta_t)
...
hist(theta_vec)
quantile(x = theta_vec, probs = c(0.2, 0.8))
mean(theta_vec)
```

Definiing a function for all age groups
```{r}
...
wrapper_MH_posterior <- function(n.sims = 1000, alpha_0 = 2, beta_0 = 20, data_frame){
  ...
  #return(list(posterior = theta_vec, quantiles = quantile(theta_vec, probs = c(0.1,0.9)), mean_posterior = mean(theta_vec)))
}

wrapper_MH_posterior(data_frame = group2_dframe, alpha_0 = 2 ,beta_0 = 20, n.sims = 1000)
...
plotting the posterior distributions for all age group
```{r}

age_frames_names <- list(group1_dframe,group2_dframe,group3_dframe,group4_dframe,
                               group5_dframe,group6_dframe,group7_dframe,group8_dframe,
                               group9_dframe,group10_dframe)


plots <- NULL
k=0
for (i in age_frames_names){
  k <- k+1
  plots[k] <- wrapper_MH_posterior(data_frame = i, n.sims = 1000, alpha_0 = 2, beta_0 = 25)
...
Comparison of normal estimation and MH algorithm for age group 0-9
